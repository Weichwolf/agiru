...
THE RULE IS THE TREE'S OWN: "`include/` IS DOCUMENTED AND `src/` IS NOT". Names and structure carry
the meaning in `src/`; a WHY belongs in the door's Doxygen if it is part of the contract, in a GATE
CASE if it is a fact about behaviour, and on the BOARD if it is a decision. Enforced rather than
stated, because a rule that only NAGS is a rule somebody is always about to get to.
...
WHAT SURVIVES, AND ONLY THERE: `///`, `//!`, `/** */` and `/*! */` in `include/`. That is the door
-- the AL surface generated code stands on -- and a door is documented where doxygen can render it,
which `make lint` counts and holds at zero. Everywhere else in `src/` nothing survives at all, which
is the point: seeing the deletion on every build is what forces code that speaks for itself.

TAKEN FROM `~/Git/outshine/test/strip-comments.py`, where the same rule has been enforced this way
for a year. The scanner is unchanged; what differs is the door list and that this tree has no
shaders to skip.
"""
import sys
import os
