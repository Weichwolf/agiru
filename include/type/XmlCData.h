...
public:
  ...
  /// \return The AL `Text`.
  /// \throws Error always -- the surface is declared, the behaviour is not (board:0035).
  /// \brief AL `XmlCData.Value()` -- the READING form, which the documentation's syntax
  ...
  static constexpr detail::XmlKind kKind = detail::XmlKind::CData;

  /// \brief A value over a node, made by the engine.
  /// \param handle The node.
  explicit XmlCData(detail::XmlHandle handle) noexcept : handle_(std::move(handle)) {}
