...
public:
  ...
  /// \throws Error always -- the surface is declared, the behaviour is not (board:0035).
  static ::agiru::XmlAttribute Create(std::string_view Name, std::string_view Value);
  ...
  /// \return The AL `XmlAttribute`.
  /// \throws Error always -- the surface is declared, the behaviour is not (board:0035).
  static ::agiru::XmlAttribute CreateNamespaceDeclaration(std::string_view Prefix,
  ...
  static constexpr detail::XmlKind kKind = detail::XmlKind::Attribute;

  /// \brief A value over a node, made by the engine.
  /// \param handle The node.
  explicit XmlAttribute(detail::XmlHandle handle) noexcept : handle_(std::move(handle)) {}
