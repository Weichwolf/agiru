...
public:
  ...
  /// \return The AL `Text`.
  /// \throws Error always -- the surface is declared, the behaviour is not (board:0035).
  /// \brief AL `XmlComment.Value()` -- the READING form, which the documentation's syntax
  /// block brackets: `[X := ] XmlComment.Value([NewX])`.
  ...
  static constexpr detail::XmlKind kKind = detail::XmlKind::Comment;

  /// \brief A value over a node, made by the engine.
  /// \param handle The node.
  explicit XmlComment(detail::XmlHandle handle) noexcept : handle_(std::move(handle)) {}
