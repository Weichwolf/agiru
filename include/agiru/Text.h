...
/// letters too; no BC document states which culture applies to a Code field, and guessing one
/// would be a silent semantic (board:0010).
...
/// a constant and an over-length assignment the only runtime check. BaseApp uses 64 distinct
...
public:
  ...
  [[nodiscard]] std::strong_ordering operator<=>(const Text &o) const {
    return value_.compare(o.value_) <=> 0;
  }

  [[nodiscard]] bool operator==(const Text &o) const { return value_ == o.value_; }
...
public:
  ...
  [[nodiscard]] std::strong_ordering operator<=>(const Code &o) const {
    return detail::CompareCode(value_, o.value_);
  }

  [[nodiscard]] bool operator==(const Code &o) const { return value_ == o.value_; }
